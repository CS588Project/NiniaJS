...
(function($){
    $.popcircle = function(selector, settings){
    	triggerFlag = true;
    	$('.blur').removeClass("disableBlur").addClass("enableBlur");
       	var config = {
	        'spacing':'10px',
	        'count':5,
	        'type':'full',
        ...
        $.each( li, function( key, value ) {
        	...
        	if(con_left==left&&con_top==top)
        	{
        		...
        		//triggerFlag = false;
        	   	$(this).animate({ top: y,left:x, opacity:1.0 }, {duration: config.time,easing: config.ease});
	        	
        	}
        	else
        	{
        		triggerFlag = false;
        		$(".blur").removeClass("enableBlur").addClass("disableBlur");
        		//menuClickFlag = false;
	        	$(this).animate({ top: con_top,left:con_left, opacity:0 }, {duration: config.time,easing: config.ease});
        	}
